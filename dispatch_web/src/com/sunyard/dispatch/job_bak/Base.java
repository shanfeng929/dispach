...
public class Base {
	...
	protected ResultSet select(String sql, Object[] elements) throws SQLException {
		setPreparedValues(sql, elements);
		//rowset.populate(new ResultSetWrapper(rs));//Invalid precision value. Cannot be less than zero
		rs = ps.executeQuery();
		return rs;
	}

	protected void setPreparedValues(String sql, Object[] elements) throws SQLException{
		if(conn == null){
...
//		}
	...
	protected long queryForLong(String sql, Object[] elements) throws SQLException{
		Object o = queryForObject(sql, elements);
		return Long.parseLong(o.toString());
	}
	protected Object queryForObject(String sql, Object[] elements) throws SQLException{
		Object o = null;
	...
	protected Task getTask(String taskDefID){
		...
		try {
			...
			if(rs.next()){
				...
				Tool.safeClose(ps);
				Tool.safeClose(rs);
				sql ="SELECT REMOTE_IP,REMOTE_USER_NAME,REMOTE_PASSWD FROM "+Table.SHELL_SERVER+" WHERE REMOTE_IP=?"; 
				rs = select(sql, new Object[]{shellID});
				if(!rs.next()){
					throw new Err("任务id["+taskDefID+"]对应的task_remote为["+shellID+"], 而表["+Table.SHELL_SERVER+"]中没有对应的服务器信息");
				}
	...
	protected HashSet<String> findPreNodes(String jobDefID, String toID) throws SQLException, DocumentException{
		Document doc = this.readXml(jobDefID);
		List<Element> arraws = doc.getRootElement().element("arrows").elements();
	...
	protected HashSet<String> findWorkFlowNextNodes(String jobDefID, String fromID) throws SQLException, DocumentException{
		Document doc = this.readXml(jobDefID);
		List<Element> arraws = doc.getRootElement().element("arrows").elements();
		HashSet<String> nodes = new HashSet<String>();
		for(Element arraw : arraws){
	...
	Document readXml(String jobDefID) throws SQLException, DocumentException{
		String sql = "SELECT XML FROM "+ Table.JOB_DEF_XML+" WHERE FLOW_ID=?";
		Object xml = this.queryForObject(sql, new Object[]{jobDefID});
		if(xml ==null)
	 ...
	 */
	protected int getNodeJoinNum(String jobDefID, String nodeID) throws SQLException, DocumentException{
		Document doc = this.readXml(jobDefID);
		Element ele = doc.getRootElement().elementByID(nodeID);
		if(ele == null){
//			return 1;
			throw new Err(Table.JOB_DEF_XML+"中flow_id["+jobDefID+"]的xml中，ID属性(ID必须大写)值为"+nodeID+"的节点不存在");
		}
