...
public class Job extends Base {
	...
	private int changeJobState(long jobID, String state) throws SQLException {
		...
		sql = "UPDATE " + Table.JOB_LOG_NAME + " SET FLOW_STATUS=? WHERE ID=?";
		elements = new Object[] { state, jobID };
//		System.out.println("changeJobState-->"+sql+"【参数】"+ elements);
		update(sql, elements);
	...
	void insertJOB(long jobID, String jobDef, long parentJobID)
		...
		update(sql, elements);
		//运行变量表的初始化
		sql="SELECT * FROM "+Table.JOB_DEF_PARAM+" WHERE FLOWID=?";
		ResultSet paraRs=select(sql,new Object[]{jobDef});
		while(paraRs.next()){
			int paraId=paraRs.getInt("ID");
			String paraName=paraRs.getString("PARANAME");
			String paraValue=paraRs.getString("PARAVALUE");
			String paraType=paraRs.getString("PARATYPE");
			sql="INSERT INTO "+Table.RUNTIME_PARAS+" (JOBID,FLOWNAME,PARAID,PARANAME,PARAVALUE,PARATYPE) VALUES(?,?,?,?,?,?)";
			update(sql,new Object[]{jobID,jobDef,paraId,paraName,paraValue,paraType});
		}
		paraRs.close();
		...
		if (parentJobID > 0)
			flowType = "3";
		sql = "INSERT INTO " + Table.JOB_LOG_NAME + "(ID,FLOW_ID,START_TIME,HIS_WORK_DATE,FLOW_TYPE,FLOW_STATUS) VALUES(?,?,?,?,?,?)";
	...
	HashSet<String> getFirstNodeDefIDs(String flowDefID) throws SQLException, DocumentException {
		HashMap<String, String> nodes = super.findWorkFlowNextNodes(flowDefID, "1");

		if (nodes.isEmpty()) {
			throw new Err("没有开始节点！");
		}
		HashSet<String> result = new HashSet<String>();
		result.add(nodes.keySet().iterator().next());
		return result;
	}
