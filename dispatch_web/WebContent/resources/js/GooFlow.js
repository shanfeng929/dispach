...
//初始化区域图的对象
	...
	if(property.haveHead){
      ...
      tmp+="<label title='"+(property.initLabelText||"newFlow_1")+"' "
		...
		if(property.headBtns.length>0){
		for(var x=0;x<property.headBtns.length;++x){
			tmp+="<a href='javascript:void(0)' id='GooFlow_head_btn_"+x+"' class='GooFlow_head_btn'><i class='ico_"+property.headBtns[x]+"'></i></a>"
		}
		}
		tmp+="</div>";
		this.$head=$(tmp);
	...
	//	this.onBtnSaveClick=null;//保存流程图按钮定义
		...
		this.$head.on("click",{inthis:this},function(e){
			...
			switch($(tar).attr("class")){
				...
				case "ico_para":	if(This.onBtnParaClick!=null)	This.onBtnParaClick();break;
				case "ico_edit": 									This.onBtnEditClick();break;
				case "ico_undo":	This.onBtnReturnClick();break;
				case "ico_redo":	This.redo();break;
				case "ico_mutiselect"	: This.onContextClick();break;
			}
		});
	}
	var toolWidth=0;
	if(property.haveTool){
		this.$bgDiv.append("<div class='GooFlow_tool'"+(property.haveHead? "":" style='margin-top:3px'")+"><div style='height:"+(height-headHeight-(property.haveHead? 7:10))+"px'  id='GooFlow_tool_div'  class='GooFlow_tool_div'></div></div>");
     ...
     +"<a href='javascript:void(0)' style='display: none;'  type='direct' class='GooFlow_tool_btn' id='"+this.$id+"_btn_direct'><i class='ico_direct'/></a>"
    );
		if(property.toolBtns&&property.toolBtns.length>0){
			tmp="<span/>";
			for(var i=0;i<property.toolBtns.length;++i){
				tmp+="<a style='display: none;'  href='javascript:void(0)' type='"+property.toolBtns[i]+"' id='"+this.$id+"_btn_"+property.toolBtns[i].split(" ")[0]+"' class='GooFlow_tool_btn'><i class='ico_small_"+property.toolBtns[i]+" ico_bar'/></a>";//加入自定义按钮
			}
	  ...
	  this.$workArea.on("click",{inthis:this},function(e){
        ...
        if(This.$lineOper.data("tid")){
          ...
          //This.$mpFrom.removeData("p");
			...
			return;
		}else if(type =='start round'){
			for(var k in This.$nodeData){
				if(This.$nodeData[k].type === 'start round') {
					This.alertTwoStartNode();
					return;
				}			
				
			}
		}
		else if(type=='end round'){
			for(var k in This.$nodeData){
				if(This.$nodeData[k].type === 'end round') {
					This.alertTwoEndNode();
					return;
				}			
		...
		var dispName ="node_"+This.$max;
		switch(type){// todo
			case 'start round':dispName='开始结点';break;
			case 'end round':dispName='结束结点';break;
			case 'linux': dispName='Linux命令节点';break;
			case 'sql': dispName ='SQL节点';break;
			case 'java': dispName ='Java调用节点';break;
			//case 'complex': dispName ='子服务节点';break;
			case 'alarm':dispName='预警节点';break;
		}
		//控件id生成
		if(dispName!='开始结点'&&dispName!='结束结点'&&dispName!='子服务节点'){
//			debugger;
			Ext.Ajax.request({
				url : basePath + '/DisPatch/maxtaskid',
				params : {
					'tempId' : This.$title,
					'type':type
				},
				success : function(result) {
					This.addNode(result.responseText,{name:dispName,id:result.responseText,left:X,top:Y,type:This.$nowType,joinNum:0});
				}
			});
		}else if(dispName=='子服务节点'){
			Ext.Ajax.request({
		...
		} else if(dispName=='开始结点'){
			This.addNode(1,{name:dispName,id:1,left:X,top:Y,type:This.$nowType});	
			
		} else if(dispName=='结束结点'){
			This.addNode(2,{name:dispName,id:2,left:X,top:Y,type:This.$nowType});	
	  ...
	  //格式function(id，type,json)：id是单元的唯一标识ID,type是单元的种类,有"node","line",种取值,json即addNode,addLine方法的第二个传参json.
	  this.onItemAdd=null;
	  //当操作某个单元（结点/线/分组块）被删除时，触发的方法，返回FALSE可阻止删除事件的发生
	  //格式function(id，type)：id是单元的唯一标识ID,type是单元的种类,有"node","line",种取值
	  this.onItemDel=null;
	  //当操作某个单元（结点/分组块）被移动时，触发的方法，返回FALSE可阻止移动事件的发生
	  //格式function(id，type,left,top)：id是单元的唯一标识ID,type是单元的种类,有"node",两种取值，线line不支持移动,left是新的左边距坐标，top是新的顶边距坐标
	  this.onItemMove=null;
	  //当操作某个单元（结点/线/分组块）被重命名时，触发的方法，返回FALSE可阻止重命名事件的发生
	  //格式function(id,name,type)：id是单元的唯一标识ID,type是单元的种类,有"node","line",种取值,name是新的名称
...
GooFlow.prototype={
	...
	//给head工具条按钮的说明文字
	setBtnRemarks:function(btnRemark){
	    if(this.$head==null)  return;
			this.$head.children("a").each(function(){
				this.title=btnRemark[$(this).attr("id").split("btn_")[1]];
			});
	...
	addNode:function(id,json){
		...
		if(json.type.indexOf(" round")<0){
			if(!json.width||json.width<60)json.width=60;
			
			/*if(!json.height||json.height<24)json.height=24; quan.shen 2016/7/28 更改遮罩高度*/
			...
			this.$nodeDom[id]=$("<div class='GooFlow_item"+mark+"' id='"+id+"' style='top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='1' style='width:"+(json.width-2)+"px;height:"+(json.height-2)+"px;'><tr><td class='ico'><i class='ico_"+json.type+" ico_node'></i></td></tr><tr><td id=name"+id+">"+json.name+"</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");
		}
		else{
			json.width=24;json.height=24;
			this.$nodeDom[id]=$("<div class='GooFlow_item item_round"+mark+"' id='"+id+"' style='top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='0'><tr><td class='ico'><i class='ico_"+json.type+"'></i></td></tr></table><div  style='display:none'><div class='rs_close'></div></div></div>");
		}
	...
	initWorkForNode:function(){
		...
		this.$workArea.delegate(".GooFlow_item","mouseup",{inthis:this},function(e){
			...
			if(lineStart&&!This.$mpTo.data("p")&&lineStart.id!='2'&&this.id!='1'){
				if(lineStart.id=='1' && this.id=='2'){
					return;
				}
		...
		this.$workArea.delegate(".GooFlow_item > .span","dblclick",{inthis:this},function(e){
			...
			var t=getElCoordinate(This.$workArea[0]);
			This.$textArea.val(oldTxt).css({display:"block",height:$(this).height(),width:60,
		...
		//.ico +   //edit by weilai
		this.$workArea.delegate("td","dblclick",{inthis:this},function(e){
			// todo
			var oldTxt=this.innerHTML;
			var This=e.data.inthis;
			var id=$(this).parents(".GooFlow_item").attr("id");
//			alert(This.$nodeData[id].type);
			var newValue = This.openNodeDialog(This.$nodeData[id],id);
			This.setName(This.$textArea.data("id"),newValue,"node");
			This.$textArea.val("").removeData("id").hide();
//			var t=getElCoordinate(This.$workArea[0]);
...
//			});
		...
		this.$workArea.delegate(".GooFlow_item > div > div[class!=rs_close]","mousedown",{inthis:this},function(e){
			...
			document.onmousemove=function(e){
				...
				if(X<60)	X=60;
				/*if(Y<24)	Y=24; quan.shen 2016/7/28 更改resize时节点最小高度*/
				if(Y<60)	Y=60;
				isMove=true;
				switch(cursor){
	...
	//把画好的整个流程图导出到一个变量中(其实也可以直接访问GooFlow对象的$nodeData,$lineData JSON属性)
	exportData:function(){
		var ret={title:this.$title,nodes:this.$nodeData,lines:this.$lineData,initNum:this.$max};
...
///////////以下为有关画线的方法
	...
	addLine:function(id,json){
		...
		//避免两个节点间不能有一条以上同向接连线
		for(var k in this.$lineData){
			if((json.from==this.$lineData[k].from&&json.to==this.$lineData[k].to||json.from==this.$lineData[k].to&&json.to==this.$lineData[k].from))
				return;
		}
		//设置$lineData[id]
		this.$lineData[id]={};
...
GooFlow.prototype.color={
		  main:"#00B4E1",
		  node:'rgba(155,155,170,0.1)',/*#A1DCEB*/
		  line:'rgba(56,146,211,0.7)',/*"#3892D3",*/
		  mark:"#ff3300",
		  mix:"#B6F700",
		  font:"#15428B"
		};
